...
class XReporterConsole {
  ...
  _itemDisplayName(qItem) {
    let ns = "";
    const ix = qItem.name.indexOf(defaults.NAMESPACE_GROUP_DELIM);
    if (ix < 0) {
      if (qItem.ns === defaults.SCRIPT_CRAFT_TASKS_NAMESPACE) {
        ns = chalk.dim.cyan(qItem.ns);
      } else if (qItem.ns) {
        ns = chalk.dim.cyan(`${qItem.ns}${defaults.NAMESPACE_GROUP_DELIM}`);
      }
    }
