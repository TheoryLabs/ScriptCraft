...
describe('xrun', function() {
  ...
  it('should fail if task is not in default namespace', () => {
    ...
    return asyncVerify(
      expectErrorToBe(
        next => xrun.run('/bar', next),
        `Task bar in namespace ${defaults.SCRIPT_CRAFT_TASKS_NAMESPACE} not found`
      )
    )
  })
  ...
  it('getNamespaces should return namespaces in order of overrides', () => {
    ...
    xrun.load('hello', {})
    expect(xrun.getNamespaces()).to.deep.equal([
      `${defaults.SCRIPT_CRAFT_TASKS_NAMESPACE}`,
      'foo',
      'blah',
      'test',
      'hello'
